...
/**
 * Transformer: Grace (grace.com) Dynamic Media / Scene7 images.
 *
 * The source uses Scene7 IS/Image URLs (e.g.
 * https://grace.scene7.com/is/image/grace/Employee-Appreciation-Day_GVETS_680x300?...)
 * for several images — in this page, the Life-at-Grace feature images are
 * linked (<a href="/page"><img src="scene7-url"></a>). A raw <img src> would
 * not survive the docx -> markdown round-trip, so we convert each DM <img>
 * into an anchor that carries the DM URL. The companion auto-block in
 * scripts/scripts.js rebuilds responsive <picture> markup client-side.
 *
 * Runs in afterTransform ONLY: block parsers run between the hooks and extract
 * <img> references into block cells (cards / carousel image cells). Rewriting
 * imgs to anchors in beforeTransform would leave parsers with no img and emit
 * empty image cells. See dm-scene7-transformer.md.
 */
