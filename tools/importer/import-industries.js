...
export default {
  transform: (payload) => {
    ...
    // Remove the pre-footer "Standard Industries" company logo, which sits in the
    // main content on these landing pages (not in the footer XF) and would
    // otherwise be swept into the tile grid. It's a global brand mark.
    ...
    main.querySelectorAll('img[alt=""], img:not([alt])').forEach((img) => {
      ...
      if (/hero\/(home|products|about|industries)/i.test(src)
    ...
    // 6. Generate sanitized path. /industries/ → /industries/index.
