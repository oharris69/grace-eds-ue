...
export default {
  transform: (payload) => {
    ...
    // main content on this page (not in the footer XF) and would otherwise be
    // swept into the tile grid. It's a global brand mark, not page content.
    ...
    main.querySelectorAll('img[alt=""], img:not([alt])').forEach((img) => {
      ...
      if (/hero\/(home|products|about)/i.test(src)
    ...
    // 6. Generate sanitized path. /about-grace/ → /about-grace/index.
